...
public class AuthenticatedUserAttribute : AuthorizeAttribute, IAsyncAuthorizationFilter
{
    ...
    {
        context.Result = new UnauthorizedObjectResult(new ErrorResponse(ErrorMessages.TOKEN_EXPIRADO));
    }

    private static void UserForbidden(AuthorizationFilterContext context)
    {
        context.Result = new UnauthorizedObjectResult(new ErrorResponse(ErrorMessages.USUARIO_SEM_PERMISSAO));
    }
}